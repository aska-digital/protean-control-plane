...
provenance event, the fallback is `rev :=` the v1 head revision value, or `1`
when no v1 revision value exists. Genesis, CAS, conflict semantics, and the
parity exclusions for `rev` and `head_event` are unchanged.
