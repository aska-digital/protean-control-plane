...
class TestGates(unittest.TestCase):
    def test_internal_names_gate_passes_on_clean_tree(self):
        result = run_gate("gates/protean-control-plane/check-internal-names.py", ROOT)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)

    def test_internal_names_gate_fails_on_injected_leak(self):
        with tempfile.TemporaryDirectory() as tmp:
            tree = os.path.join(tmp, "tree")
            shutil.copytree(os.path.join(ROOT, "gates", "protean-control-plane"),
                            os.path.join(tree, "gates", "protean-control-plane"))
            with open(os.path.join(tree, "leak.md"), "w", encoding="utf-8") as fh:
                fh.write("owner: " + fixture("leak-token") + "\n")
            result = run_gate("gates/protean-control-plane/check-internal-names.py", tree)
